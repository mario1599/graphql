...
function drawPieChart(transactions) {
  ...
  const colors = ['#6BD2FA','#5BCEFA','#51B9E1','#3F90AF','#48A4C8','#2B6278']; 
  ...
  transactions.forEach((transaction, index) => {
    const endAngle = (startAngle + (transaction.amount / totalAmount) * 2 * Math.PI);
  ...
  graphText.textContent = 'Hover to see more details'
  ...
  fetch('https://01.kood.tech/api/graphql-engine/v1/graphql', {
    ...
    .then(data => {
      if (data && data.data) {
        ...
        const upAmountBar = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
        ...
        upAmountBar.setAttribute('fill', '#5BCEFA');
        svg.appendChild(upAmountBar);

        const upAmountText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        ...
        const downAmountBar = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
        ...
        downAmountBar.setAttribute('fill', 'lightblue');
        svg.appendChild(downAmountBar);

        const downAmountText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        downAmountText.setAttribute('x', 20);
        downAmountText.setAttribute('y', 90 + upAmountBarHeight);
        downAmountText.setAttribute('fill', 'black');
        downAmountText.textContent = `Audits received: ${formatFileSize(downValue)}`;
        ...
        auditRatio.textContent = `Audit ratio: ${(upValue / downValue).toFixed(1)}`;
